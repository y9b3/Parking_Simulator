...
int main()
{
    // --- 1. INITIALISATION DES DESSINS (Indispensable !) ---
    init_modeles(); 
    ...
    goto_xy(0, 14); 
    // Tu peux décommenter ceci pour guider l'utilisateur :
    // printf("ZQSD: Déplacer | ESPACE: Changer véhicule | E: Quitter\n");
    ...
    while (key != 'e')
    {
        ...
        if (key != 0)
        {
            ...
            // --- 2. LOGIQUE ESPACE AMÉLIORÉE ---
            case ' ':
                // Si la place est vide, on met une Voiture (Type 0)
                ...
                // Si elle est occupée, on change de type (cycle)
                else {
                    all_spots[selected_spot_index].type_vehicule++;
                    
                    // Si on dépasse le type 2, on vide la place
                ...
                // On redessine la place actuelle avec le nouveau véhicule
            ...
            {
                draw_spot(all_spots[old_selection], 0); // Désélectionne l'ancienne
                draw_spot(all_spots[selected_spot_index], 1); // Sélectionne la nouvelle
            }
